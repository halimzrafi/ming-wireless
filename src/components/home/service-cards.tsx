        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {SERVICES.map((service, index) => {
            ...
            return (
              <motion.div
                key={service.id}
                className="bg-[#1E293B]/80 backdrop-blur-sm border border-white/5 rounded-2xl p-5 hover:border-[#2563EB]/30 transition-colors group"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: index * 0.08, duration: 0.4 }}
              >
                <div className="w-10 h-10 rounded-xl bg-[#2563EB]/10 flex items-center justify-center mb-3 group-hover:bg-[#2563EB]/20 transition-colors">
